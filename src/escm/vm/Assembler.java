...
public class Assembler {
  ...
  private static int getInstructionIndex(Datum instruction, String s) throws Exception {
    switch (s) {
      case "define":
        return Instruction.DEFINE;
      case "set!":
        return Instruction.SET;
      case "defined?":
        return Instruction.DEFINEDP;
      case "define-type":
        return Instruction.DEFINE_TYPE;
      case "ifn":
        return Instruction.IFN;
      case "jump":
      ...
      case "load-closure":
        return Instruction.Pseudo.LOAD_CLOSURE;
      default:
  ...
  private static Symbol parseUnaryVariadicParameter(Pair params) {
    if (params.length() != 2 || !isPeriodSymbol(params.car()))
      return null;
    Datum variadicSymbol = ((Pair) params.cdr()).car();
    if (variadicSymbol instanceof Symbol)
      return (Symbol) variadicSymbol;
    return null;
  }
  ...
  private static class SyntaxParameterComponents {
    ...
    public SyntaxParameterComponents(ArrayList<String> typeNames, ArrayList<TypeChecker.Predicate> types,
        ArrayList<Symbol> names, Symbol variadicName) {
  ...
  private static SyntaxParameterComponents parseSyntaxParameters(Pair instruction, Datum params) throws Exception {
    ...
    // Unary variadic (if the "param list" is of 2 symbols with the first as the "."
    // symbol)
      ...
      if (first instanceof Keyword) {
        ...
        if (type == null) {
          ...
          try {
            ...
            throw new Exceptionf("ASM ERROR: %s isn't valid bytecode syntax: %s", instruction.profile(),
                e.getMessage());
          }
        }
  ...
  private static Instruction assembleInstruction(Datum instruction) throws Exception {
    ...
    switch (getInstructionIndex(instruction, ((Symbol) instructionPair.car()).value())) {
      ...
      case Instruction.DEFINEDP: {
        Datum arg = getInstructionArgument(instructionPair);
        if (!(arg instanceof Symbol))
          throw new Exceptionf("ASM ERROR: \"defined?\" instruction %s must have a symbolic arg!",
              instruction.profile());
        Symbol argSymbol = (Symbol) arg;
        if (ObjectAccessChain.is(argSymbol)) {
      ...
      case Instruction.DEFINE_TYPE: {
        Datum arg = getInstructionArgument(instructionPair);
        if (!(arg instanceof Symbol))
          throw new Exceptionf("ASM ERROR: \"define-type\" instruction %s must have a symbolic arg!",
              instruction.profile());
        Symbol argSymbol = (Symbol) arg;
        if (ObjectAccessChain.is(argSymbol)) {
      ...
      case Instruction.IFN: {
        ...
        return new Instruction(Instruction.IFN, new Character(branchAmount.intValue())); // wrap java <int> in an escm
                                                                                         // <Character>
      }
      ...
      case Instruction.JUMP: {
        ...
        return new Instruction(Instruction.JUMP, new Character(branchAmount.intValue())); // wrap java <int> in an escm
                                                                                          // <Character>
      }
  ...
  public static ArrayList<Instruction> run(Datum instructionsAsData) throws Exception {
    ...
    if (instructionsAsData instanceof Nil)
      return instructions;
