...
public class ConcurrentPrimitives {
  ...
  public static class Thread extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      ...
      if (totalParams != 1 && totalParams != 2)
        throw new Exceptionf("'(thread <optional-name-string> <callable-thunk>) didn't receive 1 or 2 args: %s",
            Exceptionf.profileArgs(parameters));
      if (totalParams == 1) {
        if (!(parameters.get(0) instanceof Callable))
          throw new Exceptionf("'(thread <optional-name-string> <callable-thunk>) arg %s isn't a callable!",
              parameters.get(0).profile());
        callable = parameters.get(0);
      } else { // if(totalParams == 2)
        if (!(parameters.get(0) instanceof escm.type.String))
          throw new Exceptionf("'(thread <optional-name-string> <callable-thunk>) name 1st arg %s isn't a string!",
              parameters.get(0).profile());
        if (!(parameters.get(1) instanceof Callable))
          throw new Exceptionf(
              "'(thread <optional-name-string> <callable-thunk>) callable 2nd arg %s isn't a callable!",
              parameters.get(1).profile());
        name = ((escm.type.String) parameters.get(0)).value();
        callable = parameters.get(1);
      }
      if (name == null)
        return new escm.type.concurrent.Thread(callable);
  ...
  public static class ThreadName extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Thread))
        throw new Exceptionf("'(thread-name <thread>) didn't receive exactly 1 thread: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class ThreadId extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Thread))
        throw new Exceptionf("'(thread-id <thread>) didn't receive exactly 1 thread: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class ThreadRunnable extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Thread))
        throw new Exceptionf("'(thread-runnable <thread>) didn't receive exactly 1 thread: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class ThreadStatus extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Thread))
        throw new Exceptionf("'(thread-status <thread>) didn't receive exactly 1 thread: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class ThreadSetDaemonBang extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 2 || !(parameters.get(0) instanceof escm.type.concurrent.Thread)
          || !(parameters.get(1) instanceof Boolean))
        throw new Exceptionf("'(thread-set-daemon! <thread> <status>) didn't receive exactly 1 thread & 1 boolean: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class ThreadIsDaemon extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Thread))
        throw new Exceptionf("'(thread-daemon? <thread>) didn't receive exactly 1 thread: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class ThreadPriority extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Thread))
        throw new Exceptionf("'(thread-priority <thread>) didn't receive exactly 1 thread: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class ThreadSetPriorityBang extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 2 || !(parameters.get(0) instanceof escm.type.concurrent.Thread)
          || !(parameters.get(1) instanceof Real) || !((Real) parameters.get(1)).isInteger())
        throw new Exceptionf(
            "'(thread-set-priority! <thread> <priority>) didn't receive exactly 1 thread & 1 integer: %s",
            Exceptionf.profileArgs(parameters));
      ...
      if (priority < escm.type.concurrent.Thread.MIN_PRIORITY || priority > escm.type.concurrent.Thread.MAX_PRIORITY) {
        throw new Exceptionf(
            "'(thread-set-priority! <thread> <priority>) <priority> %d beyond system bounds [%d,%d]: %s",
            priority,
  ...
  public static class ThreadStartBang extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() < 1)
        throw new Exceptionf("'(thread-start! <thread> ...) expects at least 1 thread: %s",
            Exceptionf.profileArgs(parameters));
      ...
      for (Datum d : parameters) {
        if (!(d instanceof escm.type.concurrent.Thread))
          throw new Exceptionf("'(thread-start! <thread> ...) invalid non-thread %s given: %s", d.profile(),
              Exceptionf.profileArgs(parameters));
      ...
      for (escm.type.concurrent.Thread thread : threads) {
        if (thread.start() == false)
          failedThreads = new Pair(thread, failedThreads);
      }
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      ...
      if (totalParams == 1) {
        if (!(parameters.get(0) instanceof escm.type.concurrent.Thread))
          throw new Exceptionf("'(thread-join! <thread> <optional-millis-max-wait>) arg %s isn't a thread!",
              parameters.get(0).profile());
        thread = (escm.type.concurrent.Thread) parameters.get(0);
      } else if (totalParams == 2) {
        if (!(parameters.get(0) instanceof escm.type.concurrent.Thread))
          throw new Exceptionf("'(thread-join! <thread> <optional-millis-max-wait>) 1st arg %s isn't a thread!",
              parameters.get(0).profile());
        thread = (escm.type.concurrent.Thread) parameters.get(0);
        if (!(parameters.get(1) instanceof Real) || !((Real) parameters.get(1)).isInteger())
          throw new Exceptionf("'(thread-join! <thread> <optional-millis-max-wait>) 2nd arg %s isn't an integer!",
              parameters.get(1).profile());
        millisToWaitAtMostFor = ((Real) parameters.get(1)).intValue();
      } else {
        throw new Exceptionf("'(thread-join! <thread> <optional-millis-max-wait>) expects 1 or 2 args: %s",
            Exceptionf.profileArgs(parameters));
      }
      if (millisToWaitAtMostFor == -1)
        return Boolean.valueOf(thread.join());
  ...
  public static class ThreadIsInterrupted extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Thread))
        throw new Exceptionf("'(thread-interrupted? <thread>) didn't receive exactly 1 thread: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class ThreadInterruptBang extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Thread))
        throw new Exceptionf("'(thread-interrupt! <thread>) didn't receive exactly 1 thread: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class Sleep extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof Real) || !((Real) parameters.get(0)).isInteger())
        throw new Exceptionf("'(sleep <sleep-millis-time>) didn't receive exactly 1 integer: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class Parallel extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() == 0)
        throw new Exceptionf("'(parallel <callable> ...) expects at least 1 thunk callable: %s",
            Exceptionf.profileArgs(parameters));
      ArrayList<escm.type.concurrent.Thread> threads = new ArrayList<escm.type.concurrent.Thread>();
      for (Datum param : parameters) {
        if (!(param instanceof Callable))
          throw new Exceptionf("'(parallel <callable> ...) arg %s isn't a thunk callable: %s", param.profile(),
              Exceptionf.profileArgs(parameters));
  ...
  public static class ThreadDefineApostrophe extends Primitive {
    ...
    public Datum signature() {
      return Pair.List(
          ...
          Pair.List(new Symbol("thread-define'"), new Symbol("<thread>"), new Symbol("<symbol>"),
              new Symbol("<value>")));
    }
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      ...
      if (totalParams == 2) {
        ...
        if (!GlobalState.metaThreadDynamicEnvironment.has(varName))
          throw new Exceptionf(
              "'(thread-set!' <optional-thread> <var-name> <value>) %s isn't a variable in the meta thread dynamic environment: %s",
  ...
  public static class ThreadGetApostrophe extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      ...
      if (totalParams != 1 && totalParams != 2)
        throw new Exceptionf(
            "'(thread-get' <optional-thread> <var-name>) didn't receive 1 or 2 args (optional thread, variable symbol): %s",
            Exceptionf.profileArgs(parameters));
      if (totalParams == 1) {
        if (!(parameters.get(0) instanceof Symbol))
          throw new Exceptionf(
              "'(thread-get' <optional-thread> <var-name>) 1st arg %s isn't a symbolic variable name: %s",
              parameters.get(0).profile(), Exceptionf.profileArgs(parameters));
        Symbol varName = (Symbol) parameters.get(0);
        if (!GlobalState.metaThreadDynamicEnvironment.has(varName))
          throw new Exceptionf(
              "'(thread-get' <optional-thread> <var-name>) %s isn't a variable in the meta thread dynamic environment: %s",
              varName.value(), Exceptionf.profileArgs(parameters));
        ...
        if (!(parameters.get(1) instanceof Symbol))
          throw new Exceptionf(
              "'(thread-get' <optional-thread> <var-name>) 2nd arg %s isn't a symbolic variable name: %s",
              parameters.get(1).profile(), Exceptionf.profileArgs(parameters));
  ...
  public static class ThreadIsDefinedApostrophe extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      ...
      if (totalParams != 1 && totalParams != 2)
        throw new Exceptionf(
            "'(thread-defined?' <optional-thread> <var-name>) didn't receive 1 or 2 args (optional thread, variable symbol): %s",
            Exceptionf.profileArgs(parameters));
      if (totalParams == 1) {
        if (!(parameters.get(0) instanceof Symbol))
          throw new Exceptionf(
              "'(thread-defined?' <optional-thread> <var-name>) 1st arg %s isn't a symbolic variable name: %s",
              parameters.get(0).profile(), Exceptionf.profileArgs(parameters));
        ...
        if (!(parameters.get(1) instanceof Symbol))
          throw new Exceptionf(
              "'(thread-defined?' <optional-thread> <var-name>) 2nd arg %s isn't a symbolic variable name: %s",
              parameters.get(1).profile(), Exceptionf.profileArgs(parameters));
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      ...
      if (totalParams == 0) {
        ...
        if (!(param instanceof escm.type.String))
          throw new Exceptionf("'(mutex <optional-name-string>) 1st arg %s isn't a name string: %s", param.profile(),
              Exceptionf.profileArgs(parameters));
        return new escm.type.concurrent.Mutex(((escm.type.String) param).value());
      } else {
        throw new Exceptionf("'(mutex <optional-name-string>) didn't receive either 0 or 1 args: %s",
            Exceptionf.profileArgs(parameters));
      }
    }
  }
  ...
  public static class MutexName extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Mutex))
        throw new Exceptionf("'(mutex-name <mutex>) didn't receive exactly 1 mutex: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class MutexSpecific extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Mutex))
        throw new Exceptionf("'(mutex-specific <mutex>) didn't receive exactly 1 mutex: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class MutexSetSpecificBang extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 2 || !(parameters.get(0) instanceof escm.type.concurrent.Mutex))
        throw new Exceptionf("'(mutex-set-specific! <mutex> <value>) didn't receive exactly 1 mutex & 1 obj: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class MutexLockBang extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 && parameters.size() != 2)
        throw new Exceptionf("'(mutex-lock! <mutex> <optional-millis-timeout>) didn't receive exactly 1 or 2 args: %s",
            Exceptionf.profileArgs(parameters));
      Datum lock = parameters.get(0);
      if (!(lock instanceof escm.type.concurrent.Mutex))
        throw new Exceptionf("'(mutex-lock! <mutex> <optional-millis-timeout>) 1st arg %s isn't a mutex: %s",
            lock.profile(), Exceptionf.profileArgs(parameters));
      if (parameters.size() == 2) {
        Datum timeout = parameters.get(1);
        if (!(timeout instanceof Real) || !((Real) timeout).isInteger())
          throw new Exceptionf("'(mutex-lock! <mutex> <optional-millis-timeout>) 2nd arg %s isn't an integer: %s",
              timeout.profile(), Exceptionf.profileArgs(parameters));
  ...
  public static class MutexUnlockBang extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Mutex))
        throw new Exceptionf("'(mutex-unlock! <mutex>) didn't receive exactly 1 mutex: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class MutexIsLocked extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Mutex))
        throw new Exceptionf("'(mutex-locked? <mutex>) didn't receive exactly 1 mutex: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class MutexQueueLength extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Mutex))
        throw new Exceptionf("'(mutex-queue-length <mutex>) didn't receive exactly 1 mutex: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class MutexIsQueued extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Mutex))
        throw new Exceptionf("'(mutex-queued? <mutex>) didn't receive exactly 1 mutex: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class MutexHoldCount extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Mutex))
        throw new Exceptionf("'(mutex-hold-count <mutex>) didn't receive exactly 1 mutex: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class MutexIsHeld extends Primitive {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof escm.type.concurrent.Mutex))
        throw new Exceptionf("'(mutex-held? <mutex>) didn't receive exactly 1 mutex: %s",
            Exceptionf.profileArgs(parameters));
  ...
  // promise
  //
  // ; returns a <promise-handle> ::=
  // ; [ <promise-value> <resolved-or-rejected> <thread> ]
  ...
  public static class Promise extends Primitive {
    ...
    private static PrimitiveProcedure getResolve(Trampoline.Continuation continuation) {
      return new PrimitiveProcedure("resolve",
          new Callable() {
            ...
            public Trampoline.Bounce callWith(ArrayList<Datum> result, Trampoline.Continuation ignored)
                throws Exception {
              if (result.size() != 1)
    ...
    public static Datum logic(Callable promiseLambda) throws Exception {
      ...
      escm.type.concurrent.Thread promiseThread = new escm.type.concurrent.Thread(new Callable() {
        ...
        public Trampoline.Bounce callWith(ArrayList<Datum> ignoredArgs, Trampoline.Continuation continuation)
            throws Exception {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof Callable))
        throw new Exceptionf("'(promise <promise-lambda>) didn't receive exactly 1 callable: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class Await extends PrimitiveCallable {
    ...
    public Datum signature() {
      return Pair.List(
          Pair.List(new Symbol("await"), new Symbol("<promise>"), new Symbol("<resolve-lambda>"),
              new Symbol("<reject-lambda>")),
          Pair.List(new Symbol("await"), new Symbol("<timeout-ms>"), new Symbol("<promise>"),
              new Symbol("<resolve-lambda>"), new Symbol("<reject-lambda>")));
    }
    ...
    public Trampoline.Bounce callWith(ArrayList<Datum> parameters, Trampoline.Continuation continuation)
      ...
      if (n != 3 && n != 4)
        throw new Exceptionf(
            "'(await <optional-timeout> <promise> <resolve-lambda> <reject-lambda>) didn't receive 3 or 4 args: %s",
            Exceptionf.profileArgs(parameters));
      ...
      if (!(resolveLambda instanceof Callable))
        throw new Exceptionf(
            "'(await <optional-timeout> <promise> <resolve-lambda> <reject-lambda>) <resolve-lambda> isn't callable: %s",
            Exceptionf.profileArgs(parameters));
  ...
  // (def promises (ac->vector promises-ordered-collection)) ; Theta(1) access
  // (def still-working? #t)
  // (while (still-working?)
  // (set! still-working? #f)
  // (def idx 0)
