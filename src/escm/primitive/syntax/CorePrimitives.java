...
public class CorePrimitives {
  ...
  public static Symbol DEFINE_TYPE = new Symbol("define-type");
  ...
  public static Symbol VECTORP = new Symbol("vector?");
  ...
  public static class DefineSyntax extends PrimitiveSyntax {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 2)
        throw new Exceptionf("'(define-syntax <symbol> <callable>) didn't receive exactly 2 args: %s",
            Exceptionf.profileArgs(parameters));
      Datum name = parameters.get(0);
      if (!(name instanceof Symbol))
        throw new Exceptionf("'(define-syntax <symbol> <callable>) 1st arg %s isn't a symbol: %s", name.profile(),
            Exceptionf.profileArgs(parameters));
  ...
  // ===== parameters with a variadic parameter too, nor parsing
  //////////////////////////////////////////////////////////////////////////// <optional-docstring>,
  ...
  public static class Fn extends PrimitiveSyntaxCallable {
    ...
    public Datum signature() {
      Datum fnSignatures = Pair.List(
          ...
          Pair.List(Pair.List(new Symbol("<parameter>"), Signature.VARIADIC, new Symbol("."),
              new Symbol("<variadic-parameter>")), new Symbol("<body>"), Signature.VARIADIC),
    ...
    private static Datum parseClauseParameters(Datum params, PairBox optParams, ArrayList<Datum> parameters)
        throws Exception {
      if (!(params instanceof Pair)) {
        // should be unreachable since we know we have some optional args in <params>
        throw new Exceptionf("'(fn <optional-docstring> ((<param> ...) <body> ...) ...) invalid parameters %s: %s",
            params.profile(), Exceptionf.profileArgs(parameters));
      }
      Pair p = (Pair) params;
      Datum param = p.car();
    ...
    // Extract types/names of optional params & create <define> expressions from
    // optional params
    ...
    public static Trampoline.Bounce applyAppendMapCompile(Environment definitionEnvironment, Datum body,
        Datum compiledBody, Trampoline.Continuation continuation) throws Exception {
      if (!(body instanceof Pair))
        return continuation.run(compiledBody);
      Pair p = (Pair) body;
      return Compiler.run(p.car(), definitionEnvironment, (compiledComponent) -> () -> {
        return applyAppendMapCompile(definitionEnvironment, p.cdr(), Pair.binaryAppend(compiledBody, compiledComponent),
            continuation);
      });
    }

    private static Trampoline.Bounce compileClauseBody(Environment defEnv, Datum body, Datum compiledBody,
    ...
    // Generate series of compiled <define> expansions to convert 1 optional-param
    // clause into 2+ required-param clauses
    private static Trampoline.Bounce generateClauseDefineSequences(Environment defEnv, Keyword returnType,
        Datum reqParams, Pair optTypes, Pair optNames, Pair optDefs, Datum compiledBody, int totalOptionalParams, int i,
        Datum expandedClauses, Trampoline.Continuation continuation) throws Exception {
      if (i < 0)
        return continuation.run(expandedClauses);
        ...
        return compileClauseBody(defEnv, (Datum) optDefs.slice(i), Nil.VALUE, (compiledDefines) -> () -> {
          return generateClauseDefineSequences(
              ...
              (returnType == null
                  ...
                  : new Pair(new Pair(returnType, new Pair(params, Pair.binaryAppend(compiledDefines, compiledBody))),
                      expandedClauses)),
              continuation);
        });
      }
    ...
    private static Trampoline.Bounce getExpandedClause(Environment defEnv, Keyword returnType, Pair clause,
        ArrayList<Datum> parameters, Trampoline.Continuation continuation) throws Exception {
    ...
    // Verify the optional param is either (<name> <value>) or (<type> <name>
    // <value>)
    ...
    private static Trampoline.Bounce compileSingleClause(Environment defEnv, Keyword returnType, Pair clause,
        Trampoline.Continuation continuation) throws Exception {
      if (returnType == null) {
    ...
    // Convert all of the function's clauses w/ optional params to clauses w/
    // required params.
    // Also compile each clause's body.
    private static Trampoline.Bounce getExpandedClauses(Environment defEnv, ArrayList<Datum> parameters, int i, int n,
        Datum expandedClauses, Trampoline.Continuation continuation) throws Exception {
      if (i >= n)
        return continuation.run(expandedClauses);
      ...
      if (returnTypeAndClause == null)
        throw new Exceptionf("'(fn <optional-docstring> ((<param> ...) <body> ...) ...) invalid clause %s: %s",
            clause.profile(), Exceptionf.profileArgs(parameters));
      ...
      if (clauseHasOptionalParam(typedClause, parameters)) {
        return getExpandedClause(defEnv, returnType, typedClause, parameters, (expandedClause) -> () -> {
          return getExpandedClauses(defEnv, parameters, i + 1, n, new Pair(expandedClause, expandedClauses),
              continuation);
        });
      } else {
        return compileSingleClause(defEnv, returnType, typedClause, (compiledClause) -> () -> {
          return getExpandedClauses(defEnv, parameters, i + 1, n, new Pair(Pair.List(compiledClause), expandedClauses),
              continuation);
        });
      }
    }
    ...
    public static Trampoline.Bounce logic(Environment defEnv, ArrayList<Datum> parameters,
        Trampoline.Continuation continuation) throws Exception {
      ...
      if (n < minFnSize)
        throw new Exceptionf(
            "'(fn <optional-docstring> ((<param> ...) <body> ...) ...) needs at least 1 body clause: %s",
            Exceptionf.profileArgs(parameters));
      if (docstring == null) {
        ...
        return getExpandedClauses(defEnv, parameters, 1, n, Nil.VALUE, (expandedCompiledClauses) -> () -> {
          return continuation.run(
              Pair.List(BYTECODE, new Pair(LOAD_CLOSURE, new Pair(docstring, applyAppend(expandedCompiledClauses)))));
        });
      }
    }
  ...
  // => NOTE: Supports optional parameters via "(<param> <dflt-value>)" syntax
  //
  // NOTE: The EScheme code below doesn't account for parsing
  //////////////////////////////////////////////////////////////////////////// <optional-docstring>,
  // nor parsing keyword types.
  // =====
  //
  ...
  public static class Lambda extends PrimitiveSyntaxCallable {
    ...
    public Datum signature() {
      ...
      return Pair.List(
          ...
          Pair.List(LAMBDA, Pair.List(PARAMETER, Signature.VARIADIC, DOT, VARIADIC_PARAM), DOCSTRING, BODY,
              Signature.VARIADIC),
          Pair.List(LAMBDA,
          ...
          Pair.List(LAMBDA,
              Pair.List(PARAM, Signature.VARIADIC, Pair.List(OPTIONAL_PARAM, DEFAULT_VALUE), Signature.VARIADIC, DOT,
                  VARIADIC_PARAM),
              DOCSTRING, BODY, Signature.VARIADIC));
    }
    ...
    public static escm.util.Pair<Keyword, Datum> getReturnTypeAndParams(int n, ArrayList<Datum> parameters,
    ...
    public static Trampoline.Bounce logic(Environment defEnv, ArrayList<Datum> parameters,
      ...
      if (body instanceof Nil)
        body = new Pair(Void.VALUE, Nil.VALUE);
      parameters.clear();
      if (docstring != null)
        parameters.add(docstring);
      if (returnType != null) {
  ...
  public static class If extends PrimitiveSyntaxCallable {
    ...
    private Trampoline.Bounce logic(ArrayList<Datum> parameters, Datum condition, Datum consequence, Datum alternative,
        Trampoline.Continuation continuation) throws Exception {
          ...
          return Compiler.run(alternative, this.definitionEnvironment, (compiledAlternative) -> () -> {
            return continuation.run(
                Pair.binaryAppend(
                    ...
                    Pair.binaryAppend(
                        new Pair(Pair.List(IFN, new Exact(2 + ((OrderedCollection) compiledConsequence).length())),
                            compiledConsequence),
                        new Pair(Pair.List(JUMP, new Exact(1 + ((OrderedCollection) compiledAlternative).length())),
                            compiledAlternative))));
          });
        });
      });
    ...
    public Trampoline.Bounce callWith(ArrayList<Datum> parameters, Trampoline.Continuation continuation)
      ...
      int n = parameters.size();
      if (n < 2 || n > 3)
        throw new Exceptionf("'(if <condition> <consequence> <optional-alternative>) invalid syntax: %s",
            Exceptionf.profileArgs(parameters));
      if (n == 2) {
  ...
  public static class Begin extends PrimitiveSyntaxCallable {
    ...
    private Trampoline.Bounce iterate(ArrayList<Datum> parameters, int i, Datum compiledExpressions,
        Trampoline.Continuation continuation) throws Exception {
      if (i < 0)
        return continuation.run(new Pair(BYTECODE, compiledExpressions));
  ...
  ////////////////////////////////////////////////////////////////////////////
  // (set! <var> <value>)
  // (set! <var1> <var2> ... <varN> <N-length-list-or-vector>)
  //
  ...
  public static class SetBang extends PrimitiveSyntaxCallable {
    ...
    public Datum signature() {
      return Pair.List(
          ...
          Pair.List(SET_BANG, new Symbol("<symbol1>"), new Symbol("<symbol2>"), Signature.VARIADIC,
              new Symbol("<symbolN>"), new Symbol("<N-length-list-or-vector>")));
    }
    ...
    public static Trampoline.Bounce expandCompoundBindings(String opName, Symbol opSymbol, ArrayList<Datum> parameters,
        Trampoline.Continuation continuation) throws Exception {
      int n = parameters.size();
      Symbol hidden = UniqueSymbol.generate("compound-" + opName);
      Datum containerCreationExpression = parameters.get(n - 1);
      Datum vectorBeginExpression = Nil.VALUE;
      Datum listBeginExpression = Nil.VALUE;
      for (int i = n - 2; i >= 0; --i) {
        Datum variable = parameters.get(i);
        if (!(variable instanceof Symbol))
          throw new Exceptionf(
              "'(" + opName + " <symbol1> <symbol2> ... <symbolN> <N-length-list-or-vector>) invalid syntax: %s",
              Exceptionf.profileArgs(parameters));
        listBeginExpression = new Pair(Pair.List(opSymbol, variable, Pair.List(CAR, hidden)), listBeginExpression);
        vectorBeginExpression = new Pair(
            Pair.List(opSymbol, variable, Pair.List(hidden, new escm.type.number.Exact(i))), vectorBeginExpression);
        if (i > 0) {
          listBeginExpression = new Pair(Pair.List(SET_BANG, hidden, Pair.List(CDR, hidden)), listBeginExpression);
        }
      }
      return continuation.run(Pair.List(BEGIN, Pair.List(DEFINE, hidden, containerCreationExpression),
          Pair.List(IF, Pair.List(VECTORP, hidden), new Pair(BEGIN, vectorBeginExpression),
              new Pair(BEGIN, listBeginExpression))));
    }

    private Trampoline.Bounce compileSimpleSetBang(Datum variable, ArrayList<Datum> parameters,
        Trampoline.Continuation continuation) throws Exception {
      ...
      return Compiler.run(value, this.definitionEnvironment, (compiledValue) -> () -> {
        return continuation
            .run(Pair.binaryAppend(new Pair(BYTECODE, compiledValue), Pair.List(Pair.List(SET_BANG, variable))));
      });
    }
    ...
    public Trampoline.Bounce callWith(ArrayList<Datum> parameters, Trampoline.Continuation continuation)
      ...
      if (n <= 2)
        return compileSimpleSetBang(variable, parameters, continuation);
  ...
  ////////////////////////////////////////////////////////////////////////////
  // (define <var> <value>)
  // (define <var1> <var2> ... <varN> <N-length-list-or-vector>)
  ...
  public static class Define extends PrimitiveSyntaxCallable {
    ...
    public Datum signature() {
      return Pair.List(
          ...
          Pair.List(DEFINE, new Symbol("<symbol1>"), new Symbol("<symbol2>"), Signature.VARIADIC,
              new Symbol("<symbolN>"), new Symbol("<N-length-list-or-vector>")),
          Pair.List(DEFINE, Pair.List(new Symbol("<function-name>"), new Symbol("<parameter>"), Signature.VARIADIC),
    ...
    private Trampoline.Bounce compileSimpleDefine(ArrayList<Datum> parameters, Trampoline.Continuation continuation)
        throws Exception {
      Datum bindings = parameters.get(0);
      Datum value = parameters.size() == 2 ? parameters.get(1) : Void.VALUE;
      return Compiler.run(value, this.definitionEnvironment, (compiledValue) -> () -> {
        return continuation
            .run(Pair.binaryAppend(new Pair(BYTECODE, compiledValue), Pair.List(Pair.List(DEFINE, bindings))));
      });
    }

    private Trampoline.Bounce compileProcedureDefine(Datum bindings, int n, ArrayList<Datum> parameters,
        Trampoline.Continuation continuation) throws Exception {
      Keyword returnType = null;
      if (bindings instanceof Keyword) {
        if (n < 2)
          throw new Exceptionf("'(define (<function-name> <param> ...) <body> ...) invalid syntax: %s",
              Exceptionf.profileArgs(parameters));
      ...
      if (!(functionName instanceof Symbol))
        throw new Exceptionf("'(define (<function-name> <param> ...) <body> ...) <function-name> isn't a symbol: %s",
            Exceptionf.profileArgs(parameters));
    ...
    public Trampoline.Bounce callWith(ArrayList<Datum> parameters, Trampoline.Continuation continuation)
      ...
      if (bindings instanceof Symbol) {
        if (n <= 2)
          return compileSimpleDefine(parameters, continuation);
  ...
  public static class IsDefinedP extends PrimitiveSyntax {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 1 || !(parameters.get(0) instanceof Symbol))
        throw new Exceptionf("'(defined? <symbol>) expects exactly 1 symbol arg: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class Defn extends PrimitiveSyntax {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      ...
      if (n < 2)
        throw new Exceptionf(
            "'(defn <function-name> <optional-docstring> ((<param> ...) <body> ...) ...) invalid syntax: %s",
            Exceptionf.profileArgs(parameters));
      Datum name = parameters.get(0);
      if (!(name instanceof Symbol))
        throw new Exceptionf(
            "'(defn <function-name> <optional-docstring> ((<param> ...) <body> ...) ...) <function-name> isn't a symbol: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class Or extends PrimitiveSyntax {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      ...
      for (int i = n - 1; i >= 0; --i) {
        ...
        ifs = Pair.List(Pair.List(LAMBDA, Pair.List(cachedName), Pair.List(IF, cachedName, cachedName, ifs)),
            parameters.get(i));
      }
      return ifs;
    }
  ...
  public static class Delay extends PrimitiveSyntax {
    ...
    private static class DatumBox {
      ...
      public synchronized Datum value() {
        return value;
      }

      public synchronized void setValue(Datum v) {
        value = v;
      }
    }
    ...
    private static Datum logic(Datum delayedCode) throws Exception {
      ...
      return Pair.List(LET,
          ...
          Pair.List(LAMBDA, Nil.VALUE,
              Pair.List(IF, forced,
                  result,
                  Pair.List(BEGIN, Pair.List(SET_BANG, forced, Boolean.TRUE), Pair.List(SET_BANG, result, delayedCode),
                      result))));
    }
  ...
  public static class Cond extends PrimitiveSyntax {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      ...
      for (int i = parameters.size() - 1; i >= 0; --i) {
        Datum clause = parameters.get(i);
        if (!(clause instanceof Pair))
          throw new Exceptionf("'(cond (<condition> <expr> ...) ...) invalid syntax: %s",
              Exceptionf.profileArgs(parameters));
        Pair p = (Pair) clause;
        if (isArrowClause(p)) {
  ...
  ////////////////////////////////////////////////////////////////////////////
  // (let ((<var> <value>) ...) <body> ...)
  // (let <function-name> ((<param> <initial-value>) ...) <optional-docstring>
  //////////////////////////////////////////////////////////////////////////// <body>
  //////////////////////////////////////////////////////////////////////////// ...)
  //
  // NOTE: The EScheme code below doesn't account for parsing
  ...
  public static class Let extends PrimitiveSyntax {
    ...
    private static escm.util.Pair<Datum, Datum> getParamsAndArgs(Datum bindings, ArrayList<Datum> parameters)
        throws Exception {
      if (!(bindings instanceof Pair))
      ...
      if (!(binding instanceof Pair))
        throw new Exceptionf("'(let ((<symbol> <obj>) ...) <body> ...) invalid binding: %s",
            Exceptionf.profileArgs(parameters));
      ...
      if (bindingValue instanceof Pair)
        bindingValue = ((Pair) bindingValue).car();
    ...
    private static Datum generateNamedLet(Datum functionName, Datum bindings, Datum body, ArrayList<Datum> parameters)
        throws Exception {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() < 1)
        throw new Exceptionf("'(let ((<symbol> <obj>) ...) <body> ...) needs at least 1 arg: %s",
            Exceptionf.profileArgs(parameters));
      Datum bindings = parameters.get(0);
      ...
      if (bindings instanceof Symbol) {
        if (!(body instanceof Pair))
          throw new Exceptionf(
              "'(let <function-name> ((<param> <initial-value>) ...) <body> ...) missing parameters: %s",
              Exceptionf.profileArgs(parameters));
        Pair p = (Pair) body;
  ...
  // ; quasiquote vector
  // ((vector? lst)
  // (list (list (quote list->vector) (escm-quasiquote->quote (vector->list lst)
  //////////////////////////////////////////////////////////////////////////// level))))
  // ; quasiquote hashmap
  // ((hashmap? lst)
  // (list (list (quote list->hashmap) (escm-quasiquote->quote (hashmap->list lst)
  //////////////////////////////////////////////////////////////////////////// level))))
  ...
  // (if (= level 0)
  // (list (cadr lst))
  // (list (list (quote list) (quote (quote unquote)) (escm-quasiquote->quote
  //////////////////////////////////////////////////////////////////////////// (cadr
  //////////////////////////////////////////////////////////////////////////// lst)
  //////////////////////////////////////////////////////////////////////////// (-
  //////////////////////////////////////////////////////////////////////////// level
  ...
  // ; quasiquote vector
  // ((vector? hd)
  // (cons (list (quote list) (list (quote list->vector) (escm-quasiquote->quote
  //////////////////////////////////////////////////////////////////////////// (vector->list
  //////////////////////////////////////////////////////////////////////////// hd)
  //////////////////////////////////////////////////////////////////////////// level)))
  // (iter (cdr lst))))
  // ; quasiquote hashmap
  // ((hashmap? hd)
  // (cons (list (quote list) (list (quote list->hashmap) (escm-quasiquote->quote
  //////////////////////////////////////////////////////////////////////////// (hashmap->list
  //////////////////////////////////////////////////////////////////////////// hd)
  //////////////////////////////////////////////////////////////////////////// level)))
  // (iter (cdr lst))))
  ...
  // (iter (cdr lst)))
  // (cons (list (quote list) (escm-quasiquote->quote hd level)) ; recursively
  //////////////////////////////////////////////////////////////////////////// parse,
  //////////////////////////////////////////////////////////////////////////// in
  //////////////////////////////////////////////////////////////////////////// nested
  //////////////////////////////////////////////////////////////////////////// quasiquote
  ...
  // (iter (cdr lst)))
  // (cons (list (quote list) (escm-quasiquote->quote hd (- level 1))) ;
  //////////////////////////////////////////////////////////////////////////// recursively
  //////////////////////////////////////////////////////////////////////////// parse,
  //////////////////////////////////////////////////////////////////////////// in
  //////////////////////////////////////////////////////////////////////////// nested
    ...
    private static Datum iterate(Datum lst, int level) {
      ...
      if (isTaggedList(lstPair, UNQUOTE)) {
        if (level == 0) {
          ...
          return Pair.List(Pair.List(LIST, Pair.List(QUOTE, UNQUOTE),
              convertQuasiquoteToQuote(((Pair) lstPair.cdr()).car(), level - 1))); // *there*: recursively parse, in
                                                                                   // nested quasiquote
        }
      }
      // quasiquote vector
      ...
      if (isTaggedList(hdPair, UNQUOTE)) {
        if (level == 0) {
          ...
          return new Pair(
              Pair.List(LIST, convertQuasiquoteToQuote(hd, level)), // recursively parse, in nested quasiquote (level
                                                                    // will be decremented *there*)
              iterate(lstPair.cdr(), level));
        }
      }
      // unquote & signal should splice element
  ...
  public static class Case extends PrimitiveSyntax {
    ...
    private static Datum getConvertedClauses(Symbol cachedKey, ArrayList<Datum> parameters) throws Exception {
      ...
      for (int i = parameters.size() - 1; i > 0; --i) {
        Datum clause = parameters.get(i);
        if (!(clause instanceof Pair))
          throw new Exceptionf("'(case <value> ((<key> ...) <expr> ...) ...) invalid syntax: %s",
              Exceptionf.profileArgs(parameters));
        Pair p = (Pair) clause;
        Datum matches = p.car();
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() < 1)
        throw new Exceptionf("'(case <value> ((<key> ...) <expr> ...) ...) missing <value>: %s",
            Exceptionf.profileArgs(parameters));
      Symbol cachedKey = UniqueSymbol.generate("case-key-value");
      return Pair.List(LET, Pair.List(Pair.List(cachedKey, parameters.get(0))),
          new Pair(COND, getConvertedClauses(cachedKey, parameters)));
    }
  }
  ...
  public static class LetStar extends PrimitiveSyntax {
    ...
    private static Datum generateNestedLambdaCalls(Symbol baseLetSymbol, Datum bindings, Datum internalLet,
    ...
    public static Datum logic(String primitiveName, Symbol baseLetSymbol, ArrayList<Datum> parameters)
        throws Exception {
      if (parameters.size() < 1)
        throw new Exceptionf("'(%s ((<symbol> <value>) ...) <body> ...) missing <bindings>: %s", primitiveName,
            Exceptionf.profileArgs(parameters));
      Datum bindings = parameters.get(0);
      if (!Pair.isList(bindings))
        throw new Exceptionf("'(%s ((<symbol> <value>) ...) <body> ...) invalid <bindings>: %s", primitiveName,
            Exceptionf.profileArgs(parameters));
      Datum body = Lambda.getAllExpressionsAfter(parameters, 0);
  ...
  public static class LetRec extends PrimitiveSyntax {
    ...
    private static escm.util.Pair<Datum, Datum> getParamsDefaultAndActualSettings(Datum bindings,
        ArrayList<Datum> parameters) throws Exception {
      if (!(bindings instanceof Pair))
        return new escm.util.Pair<Datum, Datum>(Nil.VALUE, Nil.VALUE);
      Pair p = (Pair) bindings;
      Datum binding = p.car();
      if (!(binding instanceof Pair))
        throw new Exceptionf("'(letrec ((<symbol> <value>) ...) <body> ...) invalid binding: %s",
            Exceptionf.profileArgs(parameters));
      Datum bindingName = ((Pair) binding).car();
      if (!(bindingName instanceof Symbol))
        throw new Exceptionf("'(letrec ((<symbol> <value>) ...) <body> ...) invalid binding: %s",
    ...
    private static Datum convertLetrecToLet(Datum bindings, Datum body, ArrayList<Datum> parameters) throws Exception {
      ...
      return Pair.binaryAppend(new Pair(LET, new Pair(defaultAndActualSettings.first, defaultAndActualSettings.second)),
          body);
    }

    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() < 1)
        throw new Exceptionf("'(letrec ((<symbol> <value>) ...) <body> ...) missing <bindings>: %s",
            Exceptionf.profileArgs(parameters));
      Datum bindings = parameters.get(0);
      if (!Pair.isList(bindings))
        throw new Exceptionf("'(letrec ((<symbol> <value>) ...) <body> ...) invalid <bindings>: %s",
            Exceptionf.profileArgs(parameters));
      Datum body = Lambda.getAllExpressionsAfter(parameters, 0);
  ...
  public static class Unless extends PrimitiveSyntax {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() < 1)
        throw new Exceptionf("'(unless <condition> <body> ...) missing <condition>: %s",
            Exceptionf.profileArgs(parameters));
      Datum body = Lambda.getAllExpressionsAfter(parameters, 0);
  ...
  public static class When extends PrimitiveSyntax {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() < 1)
        throw new Exceptionf("'(when <condition> <body> ...) missing <condition>: %s",
            Exceptionf.profileArgs(parameters));
      Datum body = Lambda.getAllExpressionsAfter(parameters, 0);
  ...
  // (define compiled-condition (compile (car condition-returns)))
  // (define compiled-body (apply append (map compile body)))
  // (define compiled-returns (apply append (map compile (cdr
  //////////////////////////////////////////////////////////////////////////// condition-returns))))
  // (append
  ...
  public static class While extends PrimitiveSyntaxCallable {
    ...
    public Trampoline.Bounce callWith(ArrayList<Datum> parameters, Trampoline.Continuation continuation)
      ...
      if (parameters.size() < 1 || !(parameters.get(0) instanceof Pair))
        throw new Exceptionf("'(while (<condition> <return-expr> ...) <body> ...) invalid syntax: %s",
            Exceptionf.profileArgs(parameters));
      ...
      return Compiler.run(conditionReturns.car(), this.definitionEnvironment, (compiledCondition) -> () -> {
        return Fn.applyAppendMapCompile(this.definitionEnvironment, body, Nil.VALUE, (compiledBody) -> () -> {
          return Fn.applyAppendMapCompile(this.definitionEnvironment, conditionReturns.cdr(), Nil.VALUE,
              (compiledReturns) -> () -> {
                return continuation.run(Pair.binaryAppend(
                    new Pair(BYTECODE, compiledCondition),
                    Pair.binaryAppend(
                        new Pair(Pair.List(IFN, new Exact(((OrderedCollection) compiledBody).length() + 2)),
                            compiledBody),
                        Pair.binaryAppend(
                            Pair.List(
                                Pair.List(JUMP,
                                    new Exact(0 - ((OrderedCollection) compiledBody).length()
                                        - ((OrderedCollection) compiledCondition).length() - 1)),
  ...
  // (append (cons 'begin body) updates (list (cons loop-procedure-name
  //////////////////////////////////////////////////////////////////////////// vars)))))))
  ...
  public static class Do extends PrimitiveSyntax {
    ...
    public Datum signature() {
      return Pair.List(
          Pair.List(new Symbol("do"),
              Pair.List(),
              Pair.List(), new Symbol("<body>"), Signature.VARIADIC),
          Pair.List(new Symbol("do"),
          ...
          Pair.List(new Symbol("do"),
              Pair.List(
                  Pair.List(new Symbol("<var>"), new Symbol("<initial-value>"), new Symbol("<update-expression>")),
                  Signature.VARIADIC),
              Pair.List(),
              new Symbol("<body>"), Signature.VARIADIC),
          Pair.List(new Symbol("do"),
    ...
    private static escm.util.Pair<Datum, Datum> getVarsAndVals(Datum varBindings, ArrayList<Datum> parameters)
        throws Exception {
      if (!(varBindings instanceof Pair))
        return new escm.util.Pair<Datum, Datum>(Nil.VALUE, Nil.VALUE);
      Pair p = (Pair) varBindings;
      Datum binding = p.car();
      if (!(binding instanceof Pair))
        throw new Exceptionf(
            "'(do ((<var> <initial-val> <update-expr>) ...) (<break-condition> <return-expr> ...) <body> ...) invalid binding: %s",
            Exceptionf.profileArgs(parameters));
      Pair bindingPair = (Pair) binding;
      Datum value = bindingPair.cdr();
      if (value instanceof Pair)
        value = ((Pair) value).car();
    ...
    private static Datum getUpdates(Datum varBindings, ArrayList<Datum> parameters) throws Exception {
      ...
      if (bindingPair.length() == 3) {
        return new Pair(Pair.List(SET_BANG, bindingPair.car(), ((Pair) ((Pair) bindingPair.cdr()).cdr()).car()),
            getUpdates(p.cdr(), parameters));
      } else {
    ...
    private static Datum getBreakCondition(Datum breakReturns) throws Exception {
      if (breakReturns instanceof Pair)
        return ((Pair) breakReturns).car();
      return Boolean.FALSE;
    }

    private static Datum getReturnExprs(Datum breakReturns) throws Exception {
      if (breakReturns instanceof Pair)
        return ((Pair) breakReturns).cdr();
      return Nil.VALUE;
    }
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() < 2 || !Pair.isList(parameters.get(0)) || !Pair.isList(parameters.get(1)))
        throw new Exceptionf(
            "'(do ((<var> <initial-val> <update-expr>) ...) (<break-condition> <return-expr> ...) <body> ...) invalid syntax: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class For extends PrimitiveSyntax {
    ...
    private static Datum getVarDefinitions(escm.util.Pair<Datum, Datum> varsAndVals) throws Exception {
      if (!(varsAndVals.first instanceof Pair) || !(varsAndVals.second instanceof Pair))
        return Nil.VALUE;
    ...
    private static escm.util.Pair<Datum, Datum> getVarsAndVals(Datum varBindings, ArrayList<Datum> parameters)
      ...
      if (!(binding instanceof Pair))
        throw new Exceptionf(
            "'(for ((<var> <initial-val> <update-expr>) ...) (<break-condition> <return-expr> ...) <body> ...) invalid binding: %s",
            Exceptionf.profileArgs(parameters));
      Pair bindingPair = (Pair) binding;
      Datum value = bindingPair.cdr();
      if (value instanceof Pair)
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() < 2 || !Pair.isList(parameters.get(0)) || !Pair.isList(parameters.get(1)))
        throw new Exceptionf(
            "'(for ((<var> <initial-val> <update-expr>) ...) (<break-condition> <return-expr> ...) <body> ...) invalid syntax: %s",
            Exceptionf.profileArgs(parameters));
      Datum varBindings = parameters.get(0);
      escm.util.Pair<Datum, Datum> varsAndVals = getVarsAndVals(varBindings, parameters);
  ...
  // ((a op . ops)
  // (cons (quote -<>) (cons (list (list (quote lambda) (list (quote <>)) op) a)
  //////////////////////////////////////////////////////////////////////////// ops)))))
  public static class ArrowWand extends PrimitiveSyntax {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      ...
      if (n == 1) {
        ...
        return new Pair(ARROW_WAND, new Pair(
            Pair.List(Pair.List(LAMBDA, Pair.List(ARROW_WAND_ARG), parameters.get(1)), parameters.get(0)), ops));
      }
    }
  }
  ...
  public static class Curry extends PrimitiveSyntax {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      ...
      escm.util.Pair<Keyword, Datum> returnTypeAndParams = Lambda.getReturnTypeAndParams(n, parameters,
          curryErrorMessage);
      Keyword returnType = returnTypeAndParams.first;
      Datum params = returnTypeAndParams.second;
      int docstringIdx = returnType == null ? 1 : 2;
      ...
      if (isLastParameter(paramsPair)) {
        Datum foldExpr = null;
        if (returnType == null) {
          foldExpr = Pair.List(FOLD, Pair.List(LAMBDA, FA, FA), new Pair(LAMBDA, new Pair(params, body)),
              Pair.List(CONS, x, xs));
        } else {
          foldExpr = Pair.List(FOLD, Pair.List(LAMBDA, FA, FA),
              new Pair(LAMBDA, new Pair(returnType, new Pair(params, body))), Pair.List(CONS, x, xs));
        }
        if (docstring != null) {
        ...
        if (docstring != null) {
          if (returnType == null) {
            return Pair.List(LET,
                Pair.List(Pair.List(
                    curriedLambdas,
                    Pair.List(LAMBDA, firstParameter(paramsPair),
                        new Pair(CURRY, new Pair(restParameters(paramsPair), new Pair(docstring, body)))))),
          ...
          return Pair.List(LET,
              Pair.List(Pair.List(
                  ...
                  Pair.List(LAMBDA, firstParameter(paramsPair),
                      new Pair(CURRY,
                          new Pair(returnType, new Pair(restParameters(paramsPair), new Pair(docstring, body))))))),
              Pair.List(LAMBDA, new Pair(x, xs), docstring, foldExpr));
        } else {
          if (returnType == null) {
            return Pair.List(LET,
                Pair.List(Pair.List(
                    curriedLambdas,
                    Pair.List(LAMBDA, firstParameter(paramsPair),
                        new Pair(CURRY, new Pair(restParameters(paramsPair), body))))),
          ...
          return Pair.List(LET,
              Pair.List(Pair.List(
                  curriedLambdas,
                  Pair.List(LAMBDA, firstParameter(paramsPair),
                      new Pair(CURRY, new Pair(returnType, new Pair(restParameters(paramsPair), body)))))),
              Pair.List(LAMBDA, new Pair(x, xs), foldExpr));
        }
      }
    }
  ...
  public static class LetValues extends PrimitiveSyntax {
    ...
    public Datum signature() {
      return Pair.List(LET_VALUES,
          Pair.List(
              Pair.List(Pair.List(new Symbol("<variable>"), Signature.VARIADIC), new Symbol("<'values'-expression>")),
              Signature.VARIADIC),
          new Symbol("<body>"), Signature.VARIADIC);
    }
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() < 1)
        throw new Exceptionf("'(let-values (((<var> ...) <'values'-expression>) ...) <body> ...) invalid syntax: %s",
            Exceptionf.profileArgs(parameters));
      Datum bindings = parameters.get(0);
      Datum body = Lambda.getAllExpressionsAfter(parameters, 0);
      if (!(bindings instanceof Pair)) {
  ...
  //////////////////////////////////////////////////////////////////////////// ...)
  // See the below for an implementation of <with-exception-handler>, <raise>, &
  //////////////////////////////////////////////////////////////////////////// <guard>:
  ...
  public static class EscmGuardAux extends PrimitiveSyntax {
    ...
    public Datum signature() {
      return Pair.List(new Symbol("escm-guard-aux"), new Symbol("<reraise>"), new Symbol("<expression>"),
          Signature.VARIADIC);
    }
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() < 1)
        throw new Exceptionf("'(escm-guard-aux <reraise> <body> ...) invalid syntax: %s",
            Exceptionf.profileArgs(parameters));
  ...
  public static class Guard extends PrimitiveSyntax {
    ...
    private static Symbol parseRaisedVar(Datum raisedVarAndClauses, ArrayList<Datum> parameters) throws Exception {
      if (!(raisedVarAndClauses instanceof Pair))
        throw new Exceptionf("'(guard (<raised-var> (<condition> <expr> ...) ...) <body> ...) invalid syntax: %s",
            Exceptionf.profileArgs(parameters));
      Datum raisedVar = ((Pair) raisedVarAndClauses).car();
      if (!(raisedVar instanceof Symbol))
        throw new Exceptionf(
            "'(guard (<raised-var> (<condition> <expr> ...) ...) <body> ...) <raised-var> isn't a symbol: %s",
            Exceptionf.profileArgs(parameters));
                                  ...
                                  Pair.List(LAMBDA, Nil.VALUE,
                                      Pair.List(LET, Pair.List(Pair.List(raisedVar, condition)), // clauses may SET! var
                                          new Pair(ESCM_GUARD_AUX,
                                              new Pair(
                                                  Pair.List(handlerK,
                                                      Pair.List(LAMBDA, Nil.VALUE, Pair.List(RAISE, condition))),
                                                  clauses))))))))),
                  Pair.List(LAMBDA, Nil.VALUE,
  ...
  public static class DefineParameter extends PrimitiveSyntax {
    ...
    public Datum callWith(ArrayList<Datum> parameters) throws Exception {
      if (parameters.size() != 2 || !(parameters.get(0) instanceof Symbol))
        throw new Exceptionf("'(define-parameter <symbol> <obj>) invalid syntax: %s",
            Exceptionf.profileArgs(parameters));
  ...
  // (append
  // (cons (quote bytecode)
  // (compile type-keyword))
  // (list (list (quote define-type) alias-symbol))))
  public static class DefineType extends PrimitiveSyntaxCallable {
    public java.lang.String escmName() {
      return "define-type";
    }
    ...
    public Trampoline.Bounce callWith(ArrayList<Datum> parameters, Trampoline.Continuation continuation)
        throws Exception {
      if (parameters.size() != 2)
        throw new Exceptionf("'(define-type <alias-symbol> <type-keyword>) didn't receive exactly 2 args: %s",
            Exceptionf.profileArgs(parameters));
      Datum name = parameters.get(0);
      if (!(name instanceof Symbol))
        throw new Exceptionf("'(define-type <alias-symbol> <type-keyword>) 1st arg %s isn't a symbol: %s",
            name.profile(), Exceptionf.profileArgs(parameters));
      return Compiler.run(parameters.get(1), this.definitionEnvironment, (compiledKeyword) -> () -> {
        return continuation
            .run(Pair.binaryAppend(new Pair(BYTECODE, compiledKeyword), Pair.List(Pair.List(DEFINE_TYPE, name))));
      });
    }
  }
}