...
public class Vector extends Datum implements OrderedCollection, Callable {
  ...
  public Vector(int size, Datum fillValue) throws Exception {
    if (size < 0)
      throw new Exceptionf("VECTOR [CTOR]: Invalid negative <fill> size %d", size);
    value = new ArrayList<Datum>();
  ...
  private Trampoline.Bounce indexOfRecur(Callable eqp, Datum d, int i, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (this) {
      if (i >= value.size())
      ...
      return eqp.callWith(args, (isEqp) -> () -> {
        if (isEqp.isTruthy())
          return continuation.run(new Exact(i));
  ...
  public Datum pop() throws Exception {
    synchronized (this) {
      int n = value.size();
      if (n == 0)
        throw new Exceptionf("VECTOR [POP]: Can't pop an empty vector!");
  ...
  public Datum popFront() throws Exception {
    synchronized (this) {
      int n = value.size();
      if (n == 0)
        throw new Exceptionf("VECTOR [POP-FRONT]: Can't pop an empty vector!");
  ...
  public Vector subvector(int idx, int length) throws Exception {
    if (length <= 0)
      return new Vector(0, new ArrayList<Datum>());
    if (idx < 0)
    ...
    synchronized (this) {
      for (int i = 0, n = value.size(); i < n; ++i) {
        ...
        if (!(val instanceof escm.type.Character))
          throw new Exceptionf("VECTOR [TO-STRING]: Vector %s contains a non-character value %s", write(),
              val.profile());
  ...
  public Hashmap toHashmap() throws Exception {
    synchronized (this) {
      int n = value.size();
      if (n % 2 != 0)
        throw new Exceptionf("VECTOR [TO-HASHMAP]: Vector %s (size %d) doesn't have an even number of items", write(),
            n);
  ...
  public java.lang.String docstring() {
    synchronized (this) {
      ...
      if (n == 0) {
        ...
        return "Vector of length 1. Apply to index 0 to get the item. Set index 0 by applying\nto 0 and the new value. Hashcode is "
            + hashCode() + ".";
      } else {
        return "Vector of length " + n + ". Apply to any index from 0 to " + (n - 1)
  ...
  public Trampoline.Bounce callWith(ArrayList<Datum> arguments, Trampoline.Continuation continuation) throws Exception {
    ...
    if (n == 0)
      throw new Exceptionf("VECTOR [CALLABLE-GET]: Expects exactly 1 integer index arg for vector %s: %s", write(),
          Exceptionf.profileArgs(arguments));
    Datum idxDatum = arguments.get(0);
    if (!(idxDatum instanceof Real) || !((Real) idxDatum).isInteger())
      throw new Exceptionf("VECTOR [CALLABLE-GET]: Expects an integer index arg for vector %s: %s", write(),
          Exceptionf.profileArgs(arguments));
    int index = ((Real) idxDatum).intValue();
    synchronized (this) {
      if (index < 0 || index >= value.size())
        throw new Exceptionf("VECTOR [CALLABLE-GET]: Invalid index %d (size %d) for vector %s", index, value.size(),
            write());
      if (n == 1)
    ...
    throw new Exceptionf("VECTOR [CALLABLE-GET]: Expects 1 or 2 arg(s) for vector %s: %s", write(),
        Exceptionf.profileArgs(arguments));
  }
  ...
  public boolean equal(Object o) {
    if (o instanceof Vector) {
      ...
      synchronized (this) {
        synchronized (v) {
          if (v.value.size() != value.size())
            return false;
          for (int i = 0, n = value.size(); i < n; ++i)
            if (!value.get(i).equal(v.value.get(i)))
              return false;
        }
      }
      return true;
  ...
  public boolean containsType(Environment env, Predicate typePredicate) throws Exception {
    synchronized (this) {
      int n = value.size();
      for (int i = 0; i < n; ++i) {
        if (typePredicate.check(env, value.get(i)) == false)
          return false;
      }
      return true;
    }
  ...
  public Datum head() throws Exception {
    synchronized (this) {
      int n = value.size();
      if (n == 0)
        throw new Exceptionf("VECTOR [HEAD]: Can't get head of an empty vector!");
      return value.get(0);
    }
  }
  ...
  private static Trampoline.Bounce foldIter(Callable c, Datum seed, Vector v, int acPos,
  ...
  public Trampoline.Bounce fold(Callable c, Datum seed, Trampoline.Continuation continuation) throws Exception { // ->
                                                                                                                 // Datum
  ...
  private static Trampoline.Bounce FoldArrayIter(Callable c, Datum seed, AssociativeCollection[] acs, int acPos,
      Trampoline.Continuation continuation) throws Exception {
    ArrayList<Datum> args = new ArrayList<Datum>(acs.length + 1);
    args.add(seed);
    for (int i = 0; i < acs.length; ++i) {
      synchronized (acs[i]) {
        if (acPos >= acs[i].length())
          return continuation.run(seed);
  ...
  public Trampoline.Bounce FoldArray(Callable c, Datum seed, AssociativeCollection[] acs,
      Trampoline.Continuation continuation) throws Exception { // -> Datum
  ...
  private static Trampoline.Bounce mapIter(Callable c, Vector v, int acPos, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (v) {
      if (acPos >= v.length())
      ...
      args.add(v.get(acPos));
      return c.callWith(args, (mappedValue) -> () -> mapIter(c, v, acPos + 1,
          (mappedRest) -> () -> continuation.run(new Pair(mappedValue, mappedRest))));
    }
  }

  public Trampoline.Bounce map(Callable c, Trampoline.Continuation continuation) throws Exception { // ->
                                                                                                    // AssociativeCollection
    return mapIter(c, this, 0,
        (mappedList) -> () -> continuation.run(((AssociativeCollection) mappedList).toACVector()));
  }
  ...
  private static Trampoline.Bounce MapIter(Callable c, AssociativeCollection[] acs, int acPos,
      Trampoline.Continuation continuation) throws Exception {
    ...
    return c.callWith(args, (mappedValue) -> () -> MapIter(c, acs, acPos + 1,
        (mappedRest) -> () -> continuation.run(new Pair(mappedValue, mappedRest))));
  }

  public Trampoline.Bounce MapArray(Callable c, AssociativeCollection[] acs, Trampoline.Continuation continuation)
      throws Exception { // -> AssociativeCollection
    return MapIter(c, acs, 0,
        (mappedList) -> () -> continuation.run(((AssociativeCollection) mappedList).toACVector()));
  }
  ...
  private static Trampoline.Bounce forEachIter(Callable c, Vector v, int acPos, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (v) {
      if (acPos >= v.length())
        return continuation.run(Void.VALUE);
  ...
  public Trampoline.Bounce forEach(Callable c, Trampoline.Continuation continuation) throws Exception { // ->
                                                                                                        // AssociativeCollection
  ...
  private static Trampoline.Bounce ForEachIter(Callable c, AssociativeCollection[] acs, int acPos,
      Trampoline.Continuation continuation) throws Exception {
    ArrayList<Datum> args = new ArrayList<Datum>(acs.length);
    for (int i = 0; i < acs.length; ++i) {
      synchronized (acs[i]) {
  ...
  public Trampoline.Bounce ForEachArray(Callable c, AssociativeCollection[] acs, Trampoline.Continuation continuation)
      throws Exception { // -> AssociativeCollection
  ...
  private Trampoline.Bounce filterIter(Callable predicate, int acPos, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (this) {
      if (acPos >= value.size())
        return continuation.run(Nil.VALUE);
  ...
  private Trampoline.Bounce filterFrom(Callable predicate, int acPos, Trampoline.Continuation continuation)
      throws Exception { // -> AssociativeCollection
    return filterIter(predicate, acPos,
        (filteredList) -> () -> continuation.run(((AssociativeCollection) filteredList).toACVector()));
  }

  public Trampoline.Bounce filter(Callable predicate, Trampoline.Continuation continuation) throws Exception { // ->
                                                                                                               // AssociativeCollection
  ...
  private Trampoline.Bounce countIter(Callable predicate, int n, int acPos, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (this) {
      if (acPos >= value.size())
        return continuation.run(new Exact(n));
      Datum hd = value.get(acPos);
      ArrayList<Datum> args = new ArrayList<Datum>(1);
      args.add(hd);
  ...
  public Trampoline.Bounce count(Callable predicate, Trampoline.Continuation continuation) throws Exception { // ->
                                                                                                              // Exact
  ...
  private Trampoline.Bounce removeIter(Callable predicate, int acPos, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (this) {
      if (acPos >= value.size())
        return continuation.run(Nil.VALUE);
      ...
      return predicate.callWith(args, (shouldRemove) -> () -> {
        if (shouldRemove.isTruthy())
          return removeIter(predicate, acPos + 1, continuation);
        return removeIter(predicate, acPos + 1, (removedRest) -> () -> continuation.run(new Pair(hd, removedRest)));
      });
    }
  }

  public Trampoline.Bounce remove(Callable predicate, Trampoline.Continuation continuation) throws Exception { // ->
                                                                                                               // AssociativeCollection
    return removeIter(predicate, 0,
        (removedList) -> () -> continuation.run(((AssociativeCollection) removedList).toACVector()));
  }
  ...
  private Trampoline.Bounce keyIter(Callable predicate, int acPos, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (this) {
      if (acPos >= value.size()) {
        if (predicate instanceof Datum) {
          throw new Exceptionf("VECTOR [KEY]: no value in %s satisfies value predicate %s", write(),
              ((Datum) predicate).profile());
        }
      ...
      return predicate.callWith(args, (matchedKey) -> () -> {
        if (matchedKey.isTruthy())
          return continuation.run(new Exact(acPos));
    ...
    synchronized (this) {
      for (int i = 0, n = value.size(); i < n; ++i) {
        ...
        if (!(val instanceof Character))
          throw new Exceptionf("VECTOR [AC->STRING]: can't convert vector with non-char %s into a string",
              val.profile());
        sb.append(val.display());
      }
  ...
  private Trampoline.Bounce inValues(Callable eltPredicate, Datum elt, Datum values,
      Trampoline.Continuation continuation) throws Exception {
  ...
  private Trampoline.Bounce UnionArrayIter(Callable eltPredicate, AssociativeCollection[] acs, int i, Vector v, int j,
      Datum values, Trampoline.Continuation continuation) throws Exception {
    if (i >= acs.length)
      return continuation.run(values);
    synchronized (v) {
      if (j >= v.size())
        return () -> UnionArrayIter(eltPredicate, acs, i + 1,
            (i + 1 >= acs.length) ? (Vector) acs[i] : (Vector) acs[i + 1], 0, values, continuation);
      Datum elt = v.get(j);
  ...
  public Trampoline.Bounce UnionArray(Callable eltPredicate, AssociativeCollection[] acs,
      Trampoline.Continuation continuation) throws Exception {
  ...
  private Trampoline.Bounce itemIntersectsAC(Callable eltPredicate, Vector ac, int acIdx, Datum item,
      Trampoline.Continuation continuation) throws Exception {
    synchronized (ac) {
      if (acIdx >= ac.size())
        return continuation.run(Boolean.FALSE);
  ...
  private Trampoline.Bounce itemIntersects(Callable eltPredicate, AssociativeCollection[] acs, int i, Datum item,
      Trampoline.Continuation continuation) throws Exception {
    if (i >= acs.length)
      return continuation.run(Boolean.TRUE);
    return itemIntersectsAC(eltPredicate, (Vector) acs[i], 0, item, (itemInAC) -> () -> {
      if (itemInAC.isTruthy())
        return () -> itemIntersects(eltPredicate, acs, i + 1, item, continuation);
      return continuation.run(Boolean.FALSE);
    });
  }

  private Trampoline.Bounce IntersectionArrayIter(Callable eltPredicate, AssociativeCollection[] acs, int acIdx,
      Vector v, int vIdx, Datum intersectingValues, Trampoline.Continuation continuation) throws Exception {
    if (acIdx >= acs.length)
      return continuation.run(intersectingValues);
    synchronized (v) {
      if (vIdx >= v.size())
        return () -> IntersectionArrayIter(eltPredicate, acs, acIdx + 1,
            acIdx + 1 >= acs.length ? (Vector) acs[acIdx] : (Vector) acs[acIdx + 1], 0, intersectingValues,
            continuation);
        ...
        if (!isInValues.isTruthy()) {
          return itemIntersects(eltPredicate, acs, 0, elt, (intersects) -> () -> {
            if (intersects.isTruthy()) {
              return IntersectionArrayIter(eltPredicate, acs, acIdx, v, vIdx + 1, new Pair(elt, intersectingValues),
                  continuation);
            }
  ...
  public Trampoline.Bounce IntersectionArray(Callable eltPredicate, AssociativeCollection[] acs,
      Trampoline.Continuation continuation) throws Exception {
  ...
  private Trampoline.Bounce inValuesVector(Callable eltPredicate, Datum item, Vector valVector, int i,
      Trampoline.Continuation continuation) throws Exception {
  ...
  private Trampoline.Bounce binaryDifferenceArrayIter(Datum acc, Callable eltPredicate, Datum valList, Vector valVector,
      Trampoline.Continuation continuation) throws Exception {
    if (!(valList instanceof Pair))
  ...
  private Trampoline.Bounce DifferenceArrayIter(Callable eltPredicate, Datum lhs, AssociativeCollection[] acs, int i,
      Trampoline.Continuation continuation) throws Exception {
    if (i >= acs.length)
      return continuation.run(lhs);
    return binaryDifferenceArrayIter(Nil.VALUE, eltPredicate, lhs, (Vector) acs[i],
        (differenceArray) -> () -> DifferenceArrayIter(eltPredicate, differenceArray, acs, i + 1, continuation));
  }

  public Trampoline.Bounce DifferenceArray(Callable eltPredicate, AssociativeCollection[] acs,
      Trampoline.Continuation continuation) throws Exception {
  ...
  private Trampoline.Bounce binarySymmetricDifferenceArray(Callable eltPredicate, AssociativeCollection a,
      AssociativeCollection b, Trampoline.Continuation continuation) throws Exception {
    return DifferenceArray(eltPredicate, new AssociativeCollection[] { a, b }, (ab) -> () -> {
      return DifferenceArray(eltPredicate, new AssociativeCollection[] { b, a }, (ba) -> () -> {
        return UnionArray(eltPredicate,
            new AssociativeCollection[] { (AssociativeCollection) ab, (AssociativeCollection) ba }, continuation);
      });
    });
  }

  private Trampoline.Bounce SymmetricDifferenceArrayIter(Callable eltPredicate, AssociativeCollection lhs,
      AssociativeCollection[] acs, int i, Trampoline.Continuation continuation) throws Exception {
    if (i >= acs.length)
      return continuation.run((Datum) lhs);
  ...
  public Trampoline.Bounce SymmetricDifferenceArray(Callable eltPredicate, AssociativeCollection[] acs,
      Trampoline.Continuation continuation) throws Exception {
  ...
  public OrderedCollection init() throws Exception {
    synchronized (this) {
      if (value.size() == 0)
        throw new Exceptionf("VECTOR [INIT]: can't get <init> of []!");
  ...
  public Datum last() throws Exception {
    synchronized (this) {
      if (value.size() == 0)
        throw new Exceptionf("VECTOR [LAST]: can't get <last> of []!");
  ...
  private Trampoline.Bounce sliceGetLastIdx(int i, Callable continuePredicate, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (this) {
      if (i >= value.size())
        return continuation.run(new Exact(i));
  ...
  public OrderedCollection slice(int startIdx, int length) throws Exception {
    synchronized (this) {
      if (startIdx < 0 || length <= 0 || startIdx >= value.size())
        return new Vector();
  ...
  public Trampoline.Bounce slice(int startIdx, Callable continuePredicate, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (this) {
      int idx = startIdx <= 0 ? 0 : startIdx;
      if (idx >= value.size())
        return continuation.run(new Vector());
  ...
  private Trampoline.Bounce removeIter(Callable predicate, int i, boolean increasing, int mod,
      Trampoline.Continuation continuation) throws Exception {
    synchronized (this) {
      if ((increasing && i >= value.size()) || (!increasing && i < 0))
        return continuation.run(shallowCopy());
      ArrayList<Datum> args = new ArrayList<Datum>(1);
      args.add(value.get(i));
      return predicate.callWith(args, (shouldRemove) -> () -> {
        if (shouldRemove.isTruthy()) {
  ...
  private Trampoline.Bounce skipIter(Callable predicate, int i, boolean increasing, int mod,
      Trampoline.Continuation continuation) throws Exception {
    synchronized (this) {
      if ((increasing && i >= value.size()) || (!increasing && i < 0))
        return continuation.run(Boolean.FALSE);
      ...
      return predicate.callWith(args, (keepSkipping) -> () -> {
        if (keepSkipping.isTruthy())
          return skipIter(predicate, i + mod, increasing, mod, continuation);
  ...
  private static Trampoline.Bounce foldRightIter(Callable c, Datum seed, int eltIdx, Vector v,
      Trampoline.Continuation continuation) throws Exception { // -> Datum
    ArrayList<Datum> params = new ArrayList<Datum>(1);
    synchronized (v) {
      if (eltIdx >= v.value.size())
        return continuation.run(seed);
  ...
  private static Trampoline.Bounce FoldRightArray(Callable c, Datum seed, int eltIdx, AssociativeCollection[] acs,
      Trampoline.Continuation continuation) throws Exception {
  ...
  public Trampoline.Bounce FoldRightArray(Callable c, Datum seed, AssociativeCollection[] acs,
      Trampoline.Continuation continuation) throws Exception {
  ...
  private Trampoline.Bounce keyRight(Callable predicate, int idx, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (this) {
      if (idx < 0 || idx >= value.size()) {
        if (predicate instanceof Datum) {
          throw new Exceptionf("VECTOR [KEY-RIGHT]: can't find a right key from %s with predicate %s!", write(),
              ((Datum) predicate).profile());
        }
        throw new Exceptionf("VECTOR [KEY-RIGHT]: can't find a right key from %s with predicate %s!", write(),
            predicate);
      ...
      return predicate.callWith(args, (shouldGetKey) -> () -> {
        if (!shouldGetKey.isTruthy())
          return keyRight(predicate, idx - 1, continuation);
    ...
    synchronized (this) {
      return keyRight(predicate, value.size() - 1, (key) -> () -> {
        if (key instanceof Real)
          return continuation.run(key);
        if (predicate instanceof Datum) {
          throw new Exceptionf("VECTOR [KEY-RIGHT]: no value in %s satisfies value predicate %s", write(),
              ((Datum) predicate).profile());
        }
  ...
  private Trampoline.Bounce dropWhile(Callable predicate, int idx, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (this) {
      if (idx >= value.size())
        return continuation.run(new Vector());
      ...
      return predicate.callWith(args, (keepDropping) -> () -> {
        synchronized (this) {
          if (!keepDropping.isTruthy())
            return continuation.run(subvector(idx, value.size()));
  ...
  private Trampoline.Bounce dropRightWhile(Callable predicate, int idx, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (this) {
      if (idx < 0)
        return continuation.run(new Vector());
      ...
      return predicate.callWith(args, (keepDropping) -> () -> {
        synchronized (this) {
          if (!keepDropping.isTruthy())
            return continuation.run(subvector(0, idx + 1));
  ...
  private Trampoline.Bounce takeWhile(Callable predicate, int idx, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (this) {
      if (idx >= value.size())
        return continuation.run(shallowCopy());
      ...
      return predicate.callWith(args, (keepTaking) -> () -> {
        synchronized (this) {
          if (!keepTaking.isTruthy())
            return continuation.run(subvector(0, idx));
  ...
  private Trampoline.Bounce takeRightWhile(Callable predicate, int idx, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (this) {
      if (idx < 0)
        return continuation.run(shallowCopy());
      ...
      return predicate.callWith(args, (keepTaking) -> () -> {
        synchronized (this) {
          if (!keepTaking.isTruthy())
            return continuation.run(subvector(idx + 1, value.size()));
  ...
  private Trampoline.Bounce sort(Callable binaryPredicate, int idx, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (this) {
      if (idx >= value.size())
        return continuation.run(new Vector());
  ...
  private Trampoline.Bounce sorted(Callable binaryPredicate, int idx, Trampoline.Continuation continuation)
      throws Exception {
    synchronized (this) {
      if (idx + 1 >= value.size())
        return continuation.run(Boolean.TRUE);
      ...
      return binaryPredicate.callWith(args, (isEq) -> () -> {
        if (isEq.isTruthy())
          return sorted(binaryPredicate, idx + 1, continuation);
        return continuation.run(Boolean.FALSE);
      });
    }
  }
  ...
  private static Trampoline.Bounce mergeIter(Callable binaryPredicate, Vector v1, Vector v2, int i1, int i2,
      Trampoline.Continuation continuation) throws Exception {
    synchronized (v1) {
      synchronized (v2) {
        if (i1 >= v1.value.size())
          return continuation.run(v2.toListFromIndex(i2));
        if (i2 >= v2.value.size())
          return continuation.run(v1.toListFromIndex(i1));
        ...
        return binaryPredicate.callWith(args, (lt) -> () -> {
          if (lt.isTruthy()) {
            return mergeIter(binaryPredicate, v1, v2, i1 + 1, i2,
                (merged) -> () -> continuation.run(new Pair(v1Elt, merged)));
          }
          return mergeIter(binaryPredicate, v1, v2, i1, i2 + 1,
              (merged) -> () -> continuation.run(new Pair(v2Elt, merged)));
        });
      }
    }
  }

  public Trampoline.Bounce merge(Callable binaryPredicate, OrderedCollection oc, Trampoline.Continuation continuation)
      throws Exception {
    return mergeIter(binaryPredicate, this, (Vector) oc, 0, 0,
        (mergedList) -> () -> continuation.run(((AssociativeCollection) mergedList).toACVector()));
  }
  ...
  private Trampoline.Bounce skipWhileHaveDuplicates(Callable binaryPredicate, Datum d, int idx,
      Trampoline.Continuation continuation) throws Exception {
    synchronized (this) {
      if (idx >= value.size())
        return continuation.run(new Exact(idx));
  ...
  private Trampoline.Bounce deleteListNeighborDuplicates(Callable binaryPredicate, int idx,
      Trampoline.Continuation continuation) throws Exception {
    synchronized (this) {
      if (idx + 1 >= value.size())
        return continuation.run(toListFromIndex(idx));
      Datum elt = value.get(idx);
      return skipWhileHaveDuplicates(binaryPredicate, elt, idx + 1, (idxAfterDups) -> () -> {
        return deleteListNeighborDuplicates(binaryPredicate, ((Real) idxAfterDups).intValue(),
            (deldList) -> () -> continuation.run(new Pair(elt, deldList)));
      });
    }
  }

  public Trampoline.Bounce deleteNeighborDuplicates(Callable binaryPredicate, Trampoline.Continuation continuation)
      throws Exception {
    return deleteListNeighborDuplicates(binaryPredicate, 0,
        (deldList) -> () -> continuation.run(((AssociativeCollection) deldList).toACVector()));
  }
