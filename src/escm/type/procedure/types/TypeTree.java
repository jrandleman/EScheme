...
public class TypeTree {
  ...
  public static interface ValueGenerator<ParseValue> {
    ...
    public ParseValue primitiveBigInt();
    ...
    public ParseValue moduleUserType(String type, String name, ObjectAccessChain chain, Environment env)
        throws Exception;
  ...
  private static <ParseValue> Pair<ParseValue, Integer> parsePrimitive(ValueGenerator<ParseValue> gen, String name,
      int next) {
    switch (name) {
      // Any type
      case "any":
        return new Pair<ParseValue, Integer>(gen.primitiveAny(), next);

      // Numeric types
      case "num":
      case "complex":
        return new Pair<ParseValue, Integer>(gen.primitiveNum(), next);
      case "int":
        return new Pair<ParseValue, Integer>(gen.primitiveInt(), next);
      case "flo":
        return new Pair<ParseValue, Integer>(gen.primitiveFlo(), next);
      case "real":
        return new Pair<ParseValue, Integer>(gen.primitiveReal(), next);
      case "exact":
        return new Pair<ParseValue, Integer>(gen.primitiveExact(), next);
      case "inexact":
        return new Pair<ParseValue, Integer>(gen.primitiveInexact(), next);
      case "bigint":
        return new Pair<ParseValue, Integer>(gen.primitiveBigInt(), next);

      // Common atomic types
      case "str":
        return new Pair<ParseValue, Integer>(gen.primitiveStr(), next);
      case "char":
        return new Pair<ParseValue, Integer>(gen.primitiveChar(), next);
      case "key":
        return new Pair<ParseValue, Integer>(gen.primitiveKey(), next);
      case "bool":
        return new Pair<ParseValue, Integer>(gen.primitiveBool(), next);
      case "sym":
        return new Pair<ParseValue, Integer>(gen.primitiveSym(), next);
      case "void":
        return new Pair<ParseValue, Integer>(gen.primitiveVoid(), next);

      // List-base types
      case "nil":
        return new Pair<ParseValue, Integer>(gen.primitiveNil(), next);
      case "atom":
        return new Pair<ParseValue, Integer>(gen.primitiveAtom(), next);

      // Concurrency types
      case "thread":
        return new Pair<ParseValue, Integer>(gen.primitiveThread(), next);
      case "mutex":
        return new Pair<ParseValue, Integer>(gen.primitiveMutex(), next);

      // Functional types
      case "fn":
        return new Pair<ParseValue, Integer>(gen.primitiveFn(), next);
      case "procedure":
        return new Pair<ParseValue, Integer>(gen.primitiveProcedure(), next);
      case "syntax":
        return new Pair<ParseValue, Integer>(gen.primitiveSyntax(), next);

      // Object-oriented types
      case "metaobj":
        return new Pair<ParseValue, Integer>(gen.primitiveMetaobj(), next);
      case "object":
        return new Pair<ParseValue, Integer>(gen.primitiveObject(), next);
      case "class":
        return new Pair<ParseValue, Integer>(gen.primitiveClass(), next);
      case "interface":
        return new Pair<ParseValue, Integer>(gen.primitiveInterface(), next);

      // Dottable types
      case "dottable":
        return new Pair<ParseValue, Integer>(gen.primitiveDottable(), next);

      // Module types
      case "module":
        return new Pair<ParseValue, Integer>(gen.primitiveModule(), next);

      // I/O types
      case "port":
        return new Pair<ParseValue, Integer>(gen.primitivePort(), next);
      case "inport":
        return new Pair<ParseValue, Integer>(gen.primitiveIport(), next);
      case "outport":
        return new Pair<ParseValue, Integer>(gen.primitiveOport(), next);

      // Type Alias types
      case "type-alias":
        return new Pair<ParseValue, Integer>(gen.primitiveTypeAlias(), next);
  ...
  private static <ParseValue> Pair<ParseValue, Integer> parseCollection(String type, Environment env,
      ValueGenerator<ParseValue> gen, int typeLength, String name, int next) throws Exception {
    switch (name) {
      ...
      case "map": {
        ...
        if (next < typeLength && type.charAt(next) == '<') {
          ...
          if (type.charAt(parameterType.second) == ',') {
            Pair<ParseValue, Integer> valueParameterType = parseType(type, env, gen, typeLength,
                parameterType.second + 1);
  ...
  private static <ParseValue> Pair<ParseValue, Integer> parseModuleClassOrInterfaceOrAlias(String type, Environment env,
      ValueGenerator<ParseValue> gen, String name, int next) throws Exception {
  ...
  private static <ParseValue> Pair<ParseValue, Integer> parseLocalClassOrInterfaceOrAlias(String type, Environment env,
      ValueGenerator<ParseValue> gen, String name, int next) throws Exception {
    return new Pair<ParseValue, Integer>(gen.userType(type, name, env), next);
  }

  private static <ParseValue> Pair<ParseValue, Integer> parseClassOrInterfaceOrAlias(String type, Environment env,
      ValueGenerator<ParseValue> gen, String name, int next) throws Exception {
  ...
  ////////////////////////////////////////////////////////////////////////////
  // Parse Union "|" Types
  private static <ParseValue> Pair<ParseValue, Integer> parseRest(String type, Environment env,
      ValueGenerator<ParseValue> gen, int typeLength, int index, Pair<ParseValue, Integer> firstType) throws Exception {
    if (firstType.second >= typeLength)
      return firstType;
  ...
  private static <ParseValue> Pair<ParseValue, Integer> parseType(String type, Environment env,
      ValueGenerator<ParseValue> gen, int typeLength, int index) throws Exception {
    if (index >= typeLength) {
  ...
  public static <ParseValue> ParseValue walk(Keyword type, Environment env, ValueGenerator<ParseValue> gen)
      throws Exception {
    String name = type.value();
    return parseType(name, env, gen, name.length(), 1).first;
  }

  public static <ParseValue> ParseValue walk(String type, Environment env, ValueGenerator<ParseValue> gen)
      throws Exception {
