...
public class CompoundProcedure extends Procedure {
  ...
  protected static class CompileTime implements Serializable {
    ...
    // * "parameterTypeNamesLists[i][j] == null" => ith body's jth parameter has no
    // type
  ...
  public boolean isTypeSignature() {
    for (ArrayList<Instruction> body : compileTime.bodyList) {
      ...
      if (n != 0) {
        ...
        if (inst.operation != Instruction.LOAD)
          return false;
        if (!(inst.argument instanceof escm.type.Void))
          return false;
      }
    }
    return true;
  ...
  private static boolean differentTypes(String type1, Environment env1, String type2, Environment env2)
      throws Exception {
    if (mismatchedNulls(type1, type2))
      return true;
  ...
  private static boolean sameSignature(
      ...
      ArrayList<String> thatTypeSig, String thatRetType) throws Exception {
    // Same variadic status?
    if (mismatchedNulls(thisVar, thatVar))
      return false;
    // Same return type?
    if (differentTypes(thisRetType, thisEnv, thatRetType, thatEnv))
      return false;
    // Same parameter counts/types?
    int n = thisSig.size();
    if (n != thatSig.size())
      return false;
    if (mismatchedNulls(thisTypeSig, thatTypeSig))
      return false;
    if (thisTypeSig != null) {
      for (int i = 0; i < n; ++i) {
  ...
  protected CompoundProcedure(String name, SourceInformation invocationSource, Environment definitionEnvironment,
  ...
  public CompoundProcedure loadWithName(String name) {
    if (!this.name.equals(Procedure.DEFAULT_NAME))
      return this;
  ...
  public boolean eq(Object o) {
    if (!(o instanceof CompoundProcedure))
      return false;
  ...
  // Hash code
  public int hashCode() {
    return Objects.hash("compound-" + type(), System.identityHashCode(definitionEnvironment),
        System.identityHashCode(compileTime));
  }
  ...
  public Datum signature() {
    ...
    if (n == 1) {
      ...
      Datum parameterClause = clauseSignature(compileTime.getParameterTypeNames(0), compileTime.parametersList.get(0),
          compileTime.variadicParameterList.get(0));
    ...
    for (int i = n - 1; i >= 0; --i) {
      ...
      Datum parameterClause = clauseSignature(compileTime.getParameterTypeNames(i), compileTime.parametersList.get(i),
          compileTime.variadicParameterList.get(i));
      if (returnTypeName == null) {
  ...
  protected String stringifyParameters(ArrayList<String> parameterTypeNames, ArrayList<Symbol> params,
      Symbol variadic) {
    int n = params.size();
    if (n == 0 && variadic == null)
      return "()";
  ...
  protected String stringifyParameterSignatures() {
    ...
    for (int i = 0, n = compileTime.parametersList.size(); i < n; ++i) {
      ...
      sb.append(stringifyParameters(compileTime.getParameterTypeNames(i), compileTime.parametersList.get(i),
          compileTime.variadicParameterList.get(i)));
    }
    return sb.toString();
  }
  ...
  protected Trampoline.Bounce callTheFunction(int i, Environment extendedEnvironment,
  ...
  public Trampoline.Bounce callWith(ArrayList<Datum> arguments, Trampoline.Continuation continuation) throws Exception {
    return () -> {
      ...
      for (int i = 0, n = compileTime.parametersList.size(); i < n; ++i) {
        if (!validParameterArity(i, totalArguments))
          continue; // arity mismatch
        Environment extendedEnvironment = getExtendedEnvironment(i, arguments);
        if (extendedEnvironment == null)
          continue; // type mismatch
        return callTheFunction(i, extendedEnvironment, continuation);
      }
      throw new Exceptionf("Args (%s) don't match any signatures in procedure \"%s\":%s",
          Exceptionf.profileArgs(arguments), readableName(), stringifyParameterSignatures());
    };
  }
}