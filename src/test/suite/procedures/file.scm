;; Author: Jordan Randleman -- file.scm
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; VARIABLES
(def data-dir (append (absolute-path (append #path *file-separator* ".." *file-separator* ".." *file-separator* "examples" *file-separator* "procedures" *file-separator* "io")) *file-separator*))
...
(new-fp!)
(ut (make-directory fp) #t)
(set! shortlived (append fp *file-separator* (temp-file)))
(file-write shortlived 0)
...
(ut (eq? (append (path-parent io-read-file) *file-separator* "read.scm") io-read-file) #t)

(ut (eq? (append (path-parent io-read-file) *file-separator* (path-file io-read-file)) io-read-file) #t)
...
(define new-dirs (append dir1 *file-separator* dir2 *file-separator* dir3))
...
(ut (absolute-path? (append (path-parent #path) *file-separator* (temp-file))) #t)
...
(set! file2 (append file2-base *file-separator* (temp-file) *file-separator* (temp-file)))
