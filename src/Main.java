    ...
    for (int i = 0; i < args.length; ++i) {
      switch (args[i]) {
        ...
        case "--eval": {
          ...
          if (i + 1 == args.length) {
            System.err.printf(
                "\ud83d\udea8 ESCHEME ERROR \ud83d\udea8: No EScheme code given to evaluate with \"%s\"!\n", args[i]);
        ...
        case "--load": {
          ...
          if (i + 1 == args.length) {
            System.err.printf(
                "\ud83d\udea8 ESCHEME ERROR \ud83d\udea8: No filename given to load into the REPL with \"%s\"!\n",
                args[i]);
            System.err.print(COMMAND_LINE_FLAGS);
            System.exit(1);
          }
        ...
        case "--import": {
          ...
          if (i + 1 == args.length) {
            System.err.printf(
                "\ud83d\udea8 ESCHEME ERROR \ud83d\udea8: No filename given to import into the REPL with \"%s\"!\n",
                args[i]);
            System.err.print(COMMAND_LINE_FLAGS);
            System.exit(1);
          }
