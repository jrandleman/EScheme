...
[ ] WRITE LANGUAGE-WIDE UNIT TEST SUITE
    ...
    [ ] _UPLOAD TO GITHUB THE FACT THAT WE HAVE UNIT TESTING_
        * MENTION ALSO (!!! THANKS TO THE UNIT TESTS !!!):
          [X] FIXED IMPROPER ARG PROCESSING BY `with-output-to-file` AND `with-input-from-file`
          [X] FIXED A BUG WHERE `string-split` WOULD NOT RETURN THE LAST CHARACTER IN THE STRING IF GIVEN NO "SPLITTER" 2ND ARG AND STRING DIDN'T END IN A 32BIT UNICODE
          [X] CHANGED `string=?`, etc. JUST TO RETURN #t IF ONLY GIVEN 1 STRING. BEFORE, IT WOULD REQUIRE AT LEAST 2 ARGS. THIS NEW BEHAVIOR MAKES IT MORE LIKE `char=?`, etc.
          [X] FIXED A MINOR JAVA BUG IN THE STRING-FORMATTER (e.g. for `stringf` etc.)
          [X] UPDATED THE `help` DESCRIPTION FOR `meta-object` TO MENTION MODULES
          [X] IMPROVED DEBUGGING MESSAGES FOR MACROS (NOW ALSO BINDS MACRO NAME TO INTERNAL CALLABLE [IF POSSIBLE] FOR MORE DETAILED ERROR MSGS)
          [X] FIXED BUG W/ `move-file!` SOMETIMES FAILING TO PROPERLY CREATE INTERMEDIATE FILES
          [X] IMPROVED `file-` PREFIXED PRIMITIVES TO IMPROVE INTERNAL JAVA ERROR REPORTING
          [X] HAD SEVERAL THREAD PRIMTIIVES THAT USED TO THROW EXCEPTIONS NOW SIMPLY RETURN BOOLEAN SUCCESS STATUSES
          [X] ADDED THE `*min-priority*` & `*max-priority*` GLOBAL VARIABLES
          [X] FIXED BUG WHERE `read-line` WOULD FAIL GETTING THE LAST LINE OF INPUT-PORTS IF THEY DIDN'T END WITH A NEWLINE
          [X] ADDED `help` ENTRIES FOR `char-lower-case?` AND `char-alphanumeric?` (MISSING BEFORE)
          [X] ADDED IN `inexact->exact`, `exact->inexact`, `numerator`, `denominator`, `integral`, `fractional` PRIMITIVES
          [X] ADDED IN `hashmap-val?` PRIMITIVE
          [X] FIXED BUG WHERE `peek-port` WOULDN'T PROPERLY KEEP THE STREAM AT `#eof` AFTER `#eof` GETS INITIALLY DETECTED
          [X] FIXED BUG WHERE `conj` WOULD ONLY SUPPORT 2 ARGS, NOW ALSO SUPPORTS 3
          [X] UPDATED THE `number` DESCRIPTION IN THE `help` MENU
          [X] FIXED AN ISSUE WHERE COMPLEX NUMBER LITERALS THAT USED INEXACT COMPONENTS WOULD SOMETIMES FAIL
          [X] CLARIFIED INFINITY & NAN SEMANTICS IN ESCHEME'S NUMERIC TOWER
          [X] CLARIFIED MUL & EXPT SEMANTICS FOR CORNER CASES WITH `0`, `1`, `Infinity`, `-Infinity`, `NaN`
          [X] FIXED `gcd` & `lcm` BUG FOR INTEGER FLOATS
          [X] FIXED `cos`, `asin`, `acos`, `acosh`, & `atanh` FOR COMPLEX NUMBERS
          [X] FIXED `magnitude` FOR NEGATIVE REAL NUMBERS
          [X] FIXED QUASIQUOTING NESTED SEMANTICS WHEN IMMEDIATELY NESTING `quasiquote` INSTANCES: E.G. `(quasiquote (quasiquote (unquote (+ 1 2))))`
          [X] ELIMINATED FAULTY EXTRA ITEM GENERATED BY `stream->list`
          [X] FIXED BUG WHERE `from` WOULD ONLY DETECT THE OPTIONAL SOURCE-PATH-STRING IF IT WAS A STRING LITERAL
