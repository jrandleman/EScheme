...
[ ] CONSIDER HAVING ESCM PRIMITIVE ERRORS IN JAVA NOT HAVE THE "'" IN FRONT (UNLESS ESCM ERRORS DO TOO? E.G. "'(func arg) arg isn't an integer!")
...
- CONSIDER HAVING `eval` & `bytecode-eval` & `load` `load-serialized` & `serialize` (ETC. AS NEEDED) SUPPORT SANDBOXING FUNCTIONALITY TO EVAL CODE IN A SEPERATE GLOBAL ENVIRONMENT
